...
// بارگذاری تنظیمات به‌صورت امن (بدون اجرای مستقیم config.php برای جلوگیری از Parse Error)
function parse_config_constants($file) {
    ...
    $keys = [
        ...
        'BOT_TOKEN','ADMIN_ID','WEBHOOK_SECRET','SITE_URL','DEBUG_MODE','ENVIRONMENT','TIMEZONE'
    ];
    foreach ($keys as $key) {
...
if (empty($cfg)) {
    // لاگ و جلوگیری از retry بدون 500
    error_log('Webhook: config.php missing or unreadable');
    if (function_exists('error_notify_admin')) {
        error_notify_admin('webhook_config_unreadable', 'config.php missing or unreadable');
    }
    http_response_code(200);
    echo 'ok';
    exit;
...
// Define constants if not already defined (so Database/BotHelper can use them)
foreach ($cfg as $k => $v) {
    if (!defined($k)) {
        define($k, $v);
    }
