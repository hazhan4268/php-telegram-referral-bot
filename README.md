...
ربات تلگرام ارجاع و پاداش پرمیوم برای cPanel با نصب خودکار (نسخه 2.0.1)
...
### پیش‌نیازها
- ✅ Hosting با PHP 8.0+ و MySQL 5.7+/MariaDB 10+
- ✅ دسترسی به cPanel
...
1) Auto-Deploy (وب‌هوک گیت‌هاب) — پیشنهاد می‌شود
- فایل `deploy.php` را روی سرور قرار دهید و Webhook گیت‌هاب را تنظیم کنید.
- مستند کامل: `AUTO_DEPLOY_GUIDE.md`

2) اسکریپت آپدیت دستی
- از اسکریپت `update.sh` استفاده کنید (backup خودکار `config.php`).

3) جایگزینی دستی فایل‌ها
- Backup از دیتابیس و `config.php` بگیرید
- فایل‌های جدید را جایگزین کنید (به جز config.php)
- schema جدید را اجرا کنید (اگر تغییر کرده)
- کش را پاک کنید

## 🏷️ سیاست نسخه‌دهی خودکار

- فایل `VERSION` نسخه اصلی (Semantic) پروژه را نگه می‌دارد، مثل `2.0.1` و در گیت commit می‌شود.
...
- مقدار `APP_VERSION` در زمان اجرا از ترکیب این دو ساخته می‌شود:
  - اگر `BUILD` موجود باشد: `2.0.1+build.N`
...
### نسخه 2.0.1
- 🧭 ثبات اسکرول پنل ادمین: اسکرول فقط در بخش محتوا، حذف پرش و تغییرات ناگهانی ابعاد
- 📈 تثبیت ابعاد چارت و رفع پرش: ارتفاع ثابت، حذف width/height اینلاین، بهبود ریسپانسیو
- 🐞 رفع باگ دکمه‌های چارت: عدم اتکا به event سراسری و مدیریت صحیح active state
- 🏷️ نسخه‌دهی خودکار: خواندن VERSION/BUILD و افزایش Build پس از هر آپدیت (deploy/update)

### نسخه 2.0.0
- 🎨 بازطراحی کامل UI پنل ادمین (Material Design، نمودارها، انیمیشن‌ها)
- 🧩 ارتقای کامل `install.php` با UI گرافیکی و مراحل بهتر
- 🔐 بهبود امنیت: مدیریت بهتر session، CSRF، و Webhook Secret
- 🧱 پایداری: حذف `die()` از لایه دیتابیس و بهبود هندلینگ خطا در `webhook.php`
- ⚡️ AJAX برای اکشن‌های پنل و جلوگیری از تداخل خروجی‌ها
- 🚀 زیرساخت به‌روزرسانی: `deploy.php` (Webhook)، `update.sh` و `setup_git_deploy.sh`
- 🧹 پاکسازی مقادیر شخصی و استفاده از placeholder ها برای انتشار امن
